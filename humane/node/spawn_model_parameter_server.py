...
class SpawnModel():
        ...
        for i in range(0,len(sys.argv)):
          ...
          if sys.argv[i] == '-x':
            if len(sys.argv) > i+1:
              ...
              self.x_offset = float(sys.argv[i+1])
          if sys.argv[i] == '-y':
            if len(sys.argv) > i+1:
              # self.initial_xyz[1] = float(sys.argv[i+1])
              self.y_offset = float(sys.argv[i+1])
          if sys.argv[i] == '-z':
            if len(sys.argv) > i+1:
    ...
    def callSpawnService(self):
        ...
        initial_pose.position.x = rospy.get_param('~start_x',0.0) - self.x_offset
        initial_pose.position.y = rospy.get_param('~start_y',0.0) - self.y_offset
        initial_pose.position.z = rospy.get_param('~start_z',1.0)
        # convert rpy to quaternion for Pose message
        self.initial_rpy[2] = rospy.get_param('~start_yaw',0.0)
