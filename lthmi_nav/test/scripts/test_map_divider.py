...
class MapDividerTester (SyncingNode):
    ...
    def publishPdf(self):
        ...
        rospy.loginfo("%s: published pdf resoution=(%d,%d), cell_size=%f" % (rospy.get_name(), pdf.info.width, pdf.info.height, pdf.info.resolution))


    def publishPose(self):
        pose = PoseStamped()
        pose.header.stamp = rospy.Time.now()
        optimalVertex =  [27,17] #grid.genRandUnblockedVertex() #[16,5] #grid.genRandUnblockedVertex() #[23,2] #grid.genRandUnblockedVertex() #[2,10] #grid.genRandUnblockedVertex() #[15,10] #grid.genRandUnblockedVertex()
