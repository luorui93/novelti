...
start=door1   # door1   livroom1   office1   bathroom1   music1   bedroom1   door2   kitchen1   storage1   storage2   storage3   livroom2
dst=kitchen1
dst2=out
...
cmd="roslaunch lthmi_nav key.launch  start:=$start  dst:=$dst  pos:=$pos  div:=$div  use_pois:=$use_pois  ksafe:=$ksafe  mx:=$mx  view_sizes:=$view_sizes  smooth_rads:="$smooth_rads"  iarea_k:=$iarea_k  show_goal:=$show_goal dst2:=$dst2  inference_mx:=$inference_mx  reset_pdf:=$reset_pdf  pub_random_goal:=$pub_random_goal"
...

{
    ...
    echo "reset_pdf     $reset_pdf"
    echo "random_goals  $pub_random_goal"
