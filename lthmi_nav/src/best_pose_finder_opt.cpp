...
public:
    ...
    double calcAvgDist(Point point) {
        ...
        cw.calc(Point(point.x+r2a.x, point.y+r2a.y));
            ...
            ROS_INFO("%s: calculated avgDist(%d,%d)=%f", getName().c_str(), point.x, point.y, meanDist);
        #endif
        cmap.clearDist();
        return meanDist;
    }
    ...
    double slideToLocalMin() {
        ...
        ROS_INFO("%s: Started slideToLocalMin at pt=(%d,%d), avgdist=%f, r2a=(%d,%d)", getName().c_str(), pt.x, pt.y, min_dist, r2a.x, r2a.y);
        int best_nb;
        while (true) {
            best_nb = -1; //best neighbour
            #ifdef DEBUG_POSE_FINDER
                ROS_INFO("%s: looking for best neighbour around pt=(%d,%d)", getName().c_str(), pt.x, pt.y);
            #endif
            for (int nb=0; nb<8; nb++) { //iterate over neighbours
                ...
                #ifdef DEBUG_POSE_FINDER
                    ROS_INFO("%s: tried pt=(%d,%d), dist=%f, min_dist=%f", getName().c_str(), pt.x, pt.y, dist, min_dist);
                #endif
                pt.x -= NEIGHBOURS[nb][0];  pt.y -= NEIGHBOURS[nb][1];
            }
            if (best_nb>=0) {
                ...
                #ifdef DEBUG_POSE_FINDER
                    ROS_INFO("%s: move to pt=(%d,%d)", getName().c_str(), pt.x, pt.y);
                #endif
