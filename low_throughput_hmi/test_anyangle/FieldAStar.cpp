...
void FieldAStar::GetNeighbors(xyLocCont l, std::vector<xyLocCont> & neighbors, std::vector<cost> & gValues, std::vector<bool> & exists, int lookahead)
{
    ...
    float xLocs[3] = { 
        static_cast<float>(ceil(l.x - 1 - EPSILON)), 
        l.x,
        static_cast<float>(floor(static_cast<float>(l.x) + 1 + EPSILON)) 
    };
    float yLocs[3] = { 
        static_cast<float>(ceil(l.y - 1 - EPSILON)), 
        l.y, 
        static_cast<float>(floor(l.y + 1 + EPSILON))
    };
