...
# GDB config file:
# $ cd ~/ws/src/low_throughput_hmi/low_throughput_hmi/tools
# $ ln -s .gdbinit ~
...
roscd 
cd ../build/low_throughput_hmi/low_throughput_hmi
make
if [ $? == 0 ]; then
    NAME="my_main_terminal"; echo -en "\033]0;$NAME\a" #needed to keep the focus on the current terminal
    gnome-terminal --geometry=165x80+5000+40  -x sh -c "rosrun low_throughput_hmi $prog" & #pid_prog=$!
    sleep 1 
...
# ===to debug macros:===
#cd ~/ws/src/low_throughput_hmi/low_throughput_hmi/src
