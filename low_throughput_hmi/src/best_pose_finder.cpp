...
#include <low_throughput_hmi/best_pose_finder.h>
...
namespace low_throughput_hmi_cost {
    ...
    Point2D find_best_pose(
        ...
        ReachAreaInfo rai={0};
        MapRos<low_throughput_hmi::FloatMap,float> reach_area = calculate_reachability_area(map, cur_pose, max_dist, pdf, rai);
    ...
    MapRos<low_throughput_hmi::FloatMap,float> calculate_reachability_area(MapIf<int>& map, Point2D& cur_pose, int max_dist, MapIf<float>& pdf, ReachAreaInfo& rai) {
            ...
            MapRos<low_throughput_hmi::FloatMap,float> reach_area(rai.reach_size.x, rai.reach_size.y, UNREACHABLE_CELL);
