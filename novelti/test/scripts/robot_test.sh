...
cmd="roslaunch novelti key.launch  start:=$start  dst:=$dst  pos:=$pos  div:=$div  use_pois:=$use_pois  ksafe:=$ksafe  mx:=$mx  view_sizes:=$view_sizes  smooth_rads:="$smooth_rads"  iarea_k:=$iarea_k  show_goal:=$show_goal dst2:=$dst2  inference_mx:=$inference_mx  reset_pdf:=$reset_pdf  pub_random_goal:=$pub_random_goal"
