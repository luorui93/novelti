...
InferenceMatrixBad::InferenceMatrixBad(const string msg) :
        std::runtime_error(msg) {};

    
InferenceMatrix::InferenceMatrix(vector<double> infMx) :
    infMx_(infMx),
    nCmds_ ((int)floor(sqrt(infMx.size())))
{
...
double InferenceMatrix::calcEntropy(const vector<double>& probs) {
    double entropy = 0;
...
void InferenceMatrix::normalize(vector<double>& probs) {
    double sum=0;
    for (const auto& p: probs)
        sum+=p;
    for (auto& p: probs)
...
void InferenceMatrix::calcPosteriors(const vector<double>& priors, const int detCmd,
...
void InferenceMatrix::calcUpdateCoefs(const vector<double>& priors, const int detCmd,
...
string InferenceMatrix::toString(const vector<double>& probs) {
    string s;
    for (const auto& p: probs)
        s += to_string(p)+", ";
    return "["+ s +"]";
...
double InferenceMatrix::calcAvgEntropyDecrease(const vector<double>& priors) const {
...
bool InferenceMatrix::nextPriors(vector<double>& priors, double& totalProb, double delta) const {
//             cout << totalProb << endl;
    totalProb += delta;
...
double InferenceMatrix::findOptimalPriors(
