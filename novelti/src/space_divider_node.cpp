...
MapRos<novelti::IntMap, int>*    my_map = NULL;
MapRos<novelti::FloatMap,float>* my_pdf = NULL;
...
MapRos<novelti::IntMap, int>* divided_map;
...
public:
    ...
    void track_map(MapIf<int>& tm) {
        ...
        MapRos<novelti::IntMap, int> track_map_ros(tm);    
...
void act() {
    ...
    novelti::IntMap track_map_msg;
        ...
        MapRos<novelti::IntMap, int> track_map(track_map_msg);     
    //MapRos<novelti::IntMap, int> track_map(*divided_map);
...
void pdfCallback(novelti::FloatMap msg) {
    my_pdf = new MapRos<novelti::FloatMap, float>(msg);
...
void sceneCallback(novelti::Experiment msg) {
    msg.mapa.info.origin.position.x = 0;
    msg.mapa.info.origin.position.y = 0;
    my_map = new MapRos<novelti::IntMap, int>(msg.mapa);
    
    novelti::IntMap divided_map_msg;
        ...
        divided_map= new MapRos<novelti::IntMap, int>(divided_map_msg);
...
int main(int argc, char **argv) {
    ...
    ros::Publisher pub1_ =node->advertise<novelti::IntMap>("/divided_map", 1, false); //not latched
    pub1  = &pub1_;
    #ifdef DEBUG_DIVIDER
        ...
        ros::Publisher debug_dist_map_pub0 =node->advertise<novelti::IntMap>("/debug_dist_map", 1, true); //latched
        debug_dist_map_pub = &debug_dist_map_pub0;
        ros::Publisher debug_track_map_pub0 =node->advertise<novelti::IntMap>("/debug_track_map", 1, true); //latched
