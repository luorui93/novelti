...
template <class T>
setPdfUniform(std::vector<T>& pdf, const T prob_value) {
...
template <class T>
setDeterministic(std::vector<T>& pdf, int k) {
    for (auto p&: pdf) //normalize
        p = 0;
    pdf[k] = 1.0;
}
...
template <class T>
Stats calcStats(const std::vector<T>& pdf) {
    Stats stats;
...
Stats update(
        ...
        const std::vector<T>& index_map,    //input
        const std::vector<T>& coefs         //input
) {
    for (auto& p: pdf)
        if (p>=0)
            p *= coefs[index_map[k]];
